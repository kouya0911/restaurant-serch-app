...
- arrival:trueでないlegは、landmarkの有無にかかわらず、必ずdirection（右/左/直進など）の意味をtext中に明言すること。目印があることを理由に「曲がる」という表現を省略してはいけない（例:「郵便局のところで右に曲がります」「そのまま進んで、コンビニを過ぎたら左に曲がります」）。directionは渡された値をそのまま使い、AI自身の判断で変更・推測・逆転させない。
...
- 目印がどちら側にあるかという位置情報（「右手に見える」「左手にある」等の左右表現）は一切使わない（正確な位置データを渡していないため）。目印は「◯◯のところで」「◯◯を過ぎたら」「◯◯が見えたら」のように、左右に触れず存在だけを示す表現にとどめ、曲がる方向は前述の通りdirectionをそのまま明言する。目印までのピンポイント距離も絶対に言わない（そもそもJSONにも渡していない）。
